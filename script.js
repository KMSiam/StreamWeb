...
export function renderMovies(page, gridElement, movieList = movies) {
...
export function renderFilteredMovies(movieList, gridElement) {
    if (!gridElement) return;
    gridElement.innerHTML = '';
    const fragment = document.createDocumentFragment();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (prevButton) {
        prevButton.addEventListener('click', () => {
            if (currentPage > 1) {
                currentPage--;
                renderMovies(currentPage, moviesGrid);
                updateButtons();
    ...
    async function renderContinueWatching() {
        ...
        if (session) {
            ...
            const localProgress = JSON.parse(localStorage.getItem('streamweb_progress') || '{}');
            progress = Object.values(localProgress).sort((a, b) => new Date(b.updated_at) - new Date(a.updated_at));
        }
        ...
        progress.forEach(item => {
            ...
            continueItem.setAttribute('aria-label', `Continue watching ${item.title}`);
            
            const timeLeft = Math.floor(Math.random() * 40) + 5; // Mock time left
            const timeLeftText = `${timeLeft}m left`;
    ...
    function setupMovieFilters() {
        ...
        filterBtns.forEach(btn => {
            btn.addEventListener('click', () => {
                ...
                renderFilteredMovies(filteredMovies, moviesGrid);
            });
        });
    }
